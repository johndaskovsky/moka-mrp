    ...
    for (var tagnum = 0; tagnum < dom.length; tagnum++) {
      ...
      if (tag_class.length && tag_class.split(" ").indexOf('addtocalendar') != -1) {
        ...
        for (var cnum in settings['calendars']) {
          ...
          menu_links += '<li class="atcb-item"><a ' + atcb_cal_link_id + ' class="atcb-item-link" href="' + (cal_id == 'ical' && /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream ? 'webcal:' : protocol) + atc_url + cal_id + '?' + url_paramteres.join('&') + '" target="_blank">' + settings['calendars'][cnum] + '</a></li>';
        }
