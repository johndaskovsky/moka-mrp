...
//[mokamrp_detail start="yyyy-mm-dd" stop="yyyy-mm-dd" type="detail"] start can be set to "today"
function mokamrp_detail_function( $atts ){
	$start = $atts['start'];
	$stop = $atts['stop'];
	$type = $atts['type'];
	global $wpdb;
		...
		foreach($result_set as $row) {
			...
			if( $current_month != date("n", strtotime($row['start_date'])) ) {
				...
				switch ($current_month) {
				    case 1:
				        $content .= get_option('mokamrp_month_01');
				        break;
				    case 2:
				        $content .= get_option('mokamrp_month_02');
				        break;
					case 3:
				        $content .= get_option('mokamrp_month_03');
				        break;
					case 4:
				        $content .= get_option('mokamrp_month_04');
				        break;
					case 5:
				        $content .= get_option('mokamrp_month_05');
				        break;
					case 6:
				        $content .= get_option('mokamrp_month_06');
				        break;
					case 7:
				        $content .= get_option('mokamrp_month_07');
				        break;
					case 8:
				        $content .= get_option('mokamrp_month_08');
				        break;
					case 9:
				        $content .= get_option('mokamrp_month_09');
				        break;
					case 10:
				        $content .= get_option('mokamrp_month_10');
				        break;
					case 11:
				        $content .= get_option('mokamrp_month_11');
				        break;
					case 12:
				        $content .= get_option('mokamrp_month_12');
				        break;
				}
			}	
			...
			if(!empty($row['custom_html'])) {
				...
				$content .= "<br><div class=\"mokamrp_divider\"></div>";
			}
		}
	}
...
add_shortcode( 'mokamrp_detail', 'mokamrp_detail_function' );

//[mokamrp_list start="yyyy-mm-dd" stop="yyyy-mm-dd"] start can be set to "today"
function mokamrp_list_function( $atts ){
	$start = $atts['start'];
	$stop = $atts['stop'];
	global $wpdb;
	$classes = get_table_name("classes");
	...
	if(count($result_set) == 0){
		...
		$content .= "<table class=\"mokamrp_fancy_table\"><thead><tr>";
...
add_shortcode( 'mokamrp_list', 'mokamrp_list_function' );
