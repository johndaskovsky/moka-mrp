...
<?php
	...
	if (isset($_POST['submit'])) {
		check_admin_referer( 'mokamrp_edit_customer','mokamrp_edit_customer_nonce' );	
		...
		if (!empty($errors)) {
			redirect_to("admin.php?page=mokamrp_new_customer");
		}
		//End Form Validation
...
<h2>Edit Student: <?php echo $customer['first_name'] . " " . $customer['last_name']; ?>&nbsp;
		<a class="btn" href="admin.php?page=mokamrp_registration&amp;cust_id=<?php echo $customer['cust_id']; ?>"><i class="icon-ok"></i> Registration</a>&nbsp;
...
<form action="admin.php?page=mokamrp_edit_customer&amp;cust_id=<?php echo urlencode($customer['cust_id']); ?>" method="post">
	<?php wp_nonce_field( 'mokamrp_edit_customer','mokamrp_edit_customer_nonce' );  ?>
	<?php 
		$edit = true;
	...
	<div class="form-actions">
	  ...
	  <a href="admin.php?page=mokamrp_edit_customer&amp;cust_id=<?php echo urlencode($customer['cust_id']); ?>" class="btn">Cancel</a>
...
<div id="deleteCust" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-footer">	
	<form action="admin.php?page=mokamrp_delete_table_item&amp;noheader=true&amp;t=customers&i=<?php echo urlencode($customer['cust_id']); ?>" method="post">
...
<div class="tab-content">
  <div class="tab-pane active" id="current_reg">	
		<?php 
			...
			if(count($current_classes) == 0){
				...
				echo "<a class=\"btn\" href=\"admin.php?page=mokamrp_registration&amp;cust_id={$customer['cust_id']}\"><i class=\"icon-ok\"></i> Register for Classes</a>";
			}else{
				echo "<ul>";	
				...
				echo "</ul>";
				echo "<a class=\"btn\" href=\"admin.php?page=mokamrp_registration&amp;cust_id={$customer['cust_id']}\"><i class=\"icon-ok\"></i> Register for more classes</a>";
			}
		?>	
  </div>
				...
				foreach($class_history as $row) {
					...
					if(is_paid($row['reg_id'])) {
						...
						if(get_reg_balance($row['reg_id']) > 0) {
							echo " 	<a href=\"admin.php?page=mokamrp_mark_not_paid&amp;noheader=true&amp;reg_id=" . urlencode($row['reg_id']) . 
  ...
  <div class="tab-pane" id="non_class_pay">
	...
	<form class="form-inline" action="admin.php?page=mokamrp_non_class_payment&amp;noheader=true" method="post">
		<?php wp_nonce_field( 'mokamrp_non_class_payment','mokamrp_non_class_payment_nonce' ); ?>
		<div class="form-actions">
			Amount: 
