<?php check_admin_referer( 'mokamrp_print_email_z_out' );  ?>
...
<?php
	if (isset($_GET['date']) && isset($_GET['time']) && isset($_GET['group_type'])) {
		...
		if($z_out == NULL){
			...
			if(get_option('mokamrp_zout_email') != FALSE) { //If there is an email address saved in settings, then send.
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
							$headers .= 'To: '. esc_attr( get_option('mokamrp_zout_email') ) . "\r\n";
				$headers .= 'From: ' . esc_attr( get_option('mokamrp_zout_email') ) . "\r\n";
				mail(esc_attr( get_option('mokamrp_zout_email') ), "Z Out for Classes", $message, $headers);
			}
		}
	}
?>
